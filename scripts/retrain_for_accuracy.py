import os
import sys
import json
import shutil
from datetime import datetime

sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))

from models.data_loader import DataLoader
from models.trainer import train_and_save_all


def main(argv=None):
    """
    Re-train existing models with higher epochs to improve accuracy.
    Backs up old models before overwriting.
    """
    import argparse
    parser = argparse.ArgumentParser(description='Re-train models with more epochs for accuracy')
    import os
    parser.add_argument('--csv', default=os.getenv('CSV_PATH', r"C:\Users\HP\Downloads\35985678-0d79-46b4-9ed6-6f13308a1d24_201016dc9f0197de809ef278006ca85a.csv"))
    parser.add_argument('--epochs', type=int, default=50, help='Epochs for training (default 50 for accuracy)')
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--models_dir', default='trained_models')
    parser.add_argument('--backup_dir', default='trained_models_backup')
    args = parser.parse_args(argv)

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    # Backup old models
    if os.path.exists(args.models_dir):
        backup_path = f"{args.backup_dir}_{timestamp}"
        os.makedirs(args.backup_dir, exist_ok=True)
        shutil.copytree(args.models_dir, backup_path, dirs_exist_ok=True)
        print(f"✓ Backed up models to {backup_path}")
    
    loader = DataLoader(args.csv)
    loader.load_csv()
    if loader.data is None:
        print('No CSV loaded; aborting')
        return

    # Build full mapping from CSV
    df = loader.data
    has_district = 'District' in df.columns
    has_market = 'Market Name' in df.columns
    has_variety = 'Variety' in df.columns
    has_commodity = 'Commodity' in df.columns

    market_to_crops = {}
    district_to_markets = {}

    for _, row in df.iterrows():
        market = str(row['Market Name']).strip() if has_market and row.get('Market Name') else None
        district = str(row['District']).strip() if has_district and row.get('District') else None

        crop = None
        if has_variety and row.get('Variety') and str(row.get('Variety')).strip():
            crop = str(row.get('Variety')).strip()
        elif has_commodity and row.get('Commodity') and str(row.get('Commodity')).strip():
            crop = str(row.get('Commodity')).strip()

        if market and market not in ('nan', 'None'):
            if crop and crop not in ('nan', 'None'):
                market_to_crops.setdefault(market, set()).add(crop)
            if district and district not in ('nan', 'None'):
                district_to_markets.setdefault(district, set()).add(market)
            else:
                district_to_markets.setdefault('Unknown', set()).add(market)

    # Build mapping
    mapping = {}
    for district, markets in district_to_markets.items():
        crops_set = set()
        for m in markets:
            crops_set.update(market_to_crops.get(m, set()))
        mapping[district] = {
            'markets': sorted(list(markets)),
            'crops': sorted(list(crops_set))
        }

    print(f'Re-training {sum(len(v["markets"]) * len(v["crops"]) for v in mapping.values())} combinations with {args.epochs} epochs...')
    report = train_and_save_all(loader, mapping, epochs=args.epochs, batch_size=args.batch_size, out_dir=args.models_dir)

    print('\nRe-training report:')
    summary = {
        'trained': len(report.get('trained', [])),
        'skipped': len(report.get('skipped', [])),
        'failed': len(report.get('failed', []))
    }
    print(json.dumps(summary, indent=2))
    print(f"\nFull report saved to logs/retrain_{timestamp}.json")
    os.makedirs('logs', exist_ok=True)
    with open(f"logs/retrain_{timestamp}.json", 'w') as f:
        json.dump(report, f, indent=2)


if __name__ == '__main__':
    main()
